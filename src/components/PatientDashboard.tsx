...
export default function PatientDashboard({ user }: PatientDashboardProps) {
  ...
  // Session timeout hook with HIPAA-compliant 15-minute timeout
  const {
    showWarning,
    remainingTime,
    extendSession,
    ...
    timeoutMinutes: 15, // HIPAA recommended timeout
    warningMinutes: 2,  // 2-minute warning
    onTimeout: handleSessionTimeout,
    onWarning: () => {
      // Could add additional warning logic here
    }
  });
  
  async function handleSessionTimeout() {
    await handleSignOut('session_timeout');
  }

  const handleSignOut = async (reason: string = 'user_action') => {
    setLoading(true);
    setError(null);
    ...
    try {
      ...
      const apiError: APIError = {
        message: err instanceof Error ? err.message : 'Error signing out. Please try again.',
        code: 'SIGNOUT_ERROR'
      };
      setError(apiError);
  ...
  return (
    <ErrorBoundary>
      <SessionTimeoutModal
        ...
        onSignOut={() => handleSignOut('user_timeout_choice')}
      />
        ...
        <div className="container-max">
          <div className="flex items-center justify-between py-4">
            ...
            <div className="flex items-center space-x-4">
              ...
              <button
                onClick={() => handleSignOut('user_action')}
                disabled={loading}
